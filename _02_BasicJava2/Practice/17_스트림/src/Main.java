...
//      1-4. 스트림 generate
        ...
        Stream streamGenerate = Stream.generate(() -> "abc").limit(3); //3번 반복해서 생성
...
//      1-5. 스트림 iterate
        ...
        //n제곱
        Stream streamIterate2 = Stream.iterate(2, n -> n * n).limit(3);
        streamIterate2.forEach(System.out::println);


//      1-6. 기본 타입 스트림
        System.out.println("== 기본타입 스트림 ==");
        IntStream intStream = IntStream.range(1, 5); //range(1, 5) : 1~4까지, rangeClose(1, 5) : 1~5까지
...
//      2-3. Sorting
        ...
        IntStream intStream4 = IntStream.builder().add(5).add(1).add(2).add(3).add(4).add(2).build();
...
//      3-3. reduce : 연쇄적으로 이전항과 다음항을 연산해서 결과를 뽑아낼 때 사용
        ...
        //팩토리얼
        IntStream streamFactorial = IntStream.rangeClosed(1, 5);
        int factorial = streamFactorial.reduce((x, y) -> x * y).getAsInt(); // 1 * 2, (1 * 2) * 3, (1 * 2 * 3) * 4....

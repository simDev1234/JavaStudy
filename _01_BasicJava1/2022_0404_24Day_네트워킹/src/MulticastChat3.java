...
public class MulticastChat3 extends JFrame {
	...
	JTextField jtf_group_name; //그룹이름
...
//			}
	...
	private void init_inputs() {
		...
		jtf_group_name = new JTextField();
		jtf_group_name.setFont(font);
		p0.add(jtf_address);
		p0.add(jtf_group_name);
	...
	private void on_button_leave() {
		...
		//(기) 가입 여부 확인 
		if(groupList.contains(groupList.get(index))) {
			...
			if(result==JOptionPane.YES_OPTION) {
				...
				try {
					//네트워크상 탈퇴
					InetAddress ia = InetAddress.getByName(groupList.get(index));
	...
	protected void on_button_join() {
		...
		//그룹이름
		String group_name = jtf_group_name.getText().trim();
		if(group_name.isEmpty()) {
			JOptionPane.showMessageDialog(this, "가입할 그룹명을 입력하세요.");
			jtf_group_name.setText("");
			return;
		}
		...
		try {
			...
			//가입된 그룹명을 ArrayList에 넣기
			groupNameList.add(group_name);
	...
	private void update_group_list() {
		...
		*/
		...
		jlist_group.setListData(groupNameArr);
	}
